...
export const ProjectsSection: React.FC = () => {
  ...
  return (
    <section
      ref={containerRef}
      id="projects"
      className={`px-5 sm:px-8 md:px-10 py-20 sm:py-24 md:py-32 relative z-20 overflow-x-clip select-none transition-colors duration-500 ${theme === 'dark' ? 'bg-[#0C0C0C] text-[#D7E2EA]' : 'bg-[#F8FAFC] text-[#0F172A]'
        }`}
    >
      <div className="max-w-6xl mx-auto flex flex-col items-center">
        ...
        <div className="relative flex flex-col gap-12 sm:gap-16 md:gap-24 w-full">
...
const ProjectCard: React.FC<CardProps> = ({ project, index, totalCards, scrollYProgress }) => {
  ...
  return (
    <div
      ...
      className="h-[80vh] sm:h-[85vh] min-h-[500px] max-h-[760px] sticky top-24 md:top-32 flex justify-center items-stretch w-full mb-12 sm:mb-16 md:mb-24"
      style={{
        top: `${96 + index * 28}px`
      }}
    >
      <motion.div
        ...
        className={`w-full border-2 rounded-[40px] sm:rounded-[50px] md:rounded-[60px] p-5 sm:p-7 md:p-9 flex flex-col justify-between overflow-hidden transition-[background-color,border-color,box-shadow] duration-300 relative shadow-2xl ${theme === 'dark'
      ...
      >
        ...
        <div className={`flex justify-between items-center gap-4 border-b pb-5 z-20 ${theme === 'dark' ? 'border-[#D7E2EA]/15' : 'border-[#0F172A]/15'
          }`}>
          <div className="flex items-center gap-3 sm:gap-5 md:gap-7">
            ...
            <div className="flex flex-col text-left">
              ...
              <h3 className={`uppercase text-base sm:text-xl md:text-2xl lg:text-3xl font-black tracking-tight leading-tight mt-0.5 ${theme === 'dark' ? 'text-[#D7E2EA]' : 'text-slate-950'
                }`}>
                {project.title}
              </h3>
              {/* Tech Tags */}
              <div className="flex flex-wrap gap-1.5 mt-2">
                {project.tags.map((tag, i) => (
                  <span
                    key={i}
                    className={`text-[9px] sm:text-[10px] font-extrabold uppercase px-2.5 py-0.5 rounded-full border ${theme === 'dark'
        ...
        {/* Middle Image Cards Row with 3D Tilt Hover Effects */}
        <div className="my-4 flex-grow flex items-center justify-center gap-3 sm:gap-5 overflow-x-auto select-none py-2 px-1 z-20">
          {project.images.map((imgSrc, i) => (
            <motion.div
              key={i}
              whileHover={{ scale: 1.06, y: -8, rotateZ: i % 2 === 0 ? 1.5 : -1.5 }}
              transition={{ type: 'spring', stiffness: 300, damping: 18 }}
              className={`flex-1 min-w-[200px] max-w-[340px] h-[180px] sm:h-[240px] md:h-[290px] rounded-2xl sm:rounded-3xl border-2 overflow-hidden shrink-0 shadow-xl relative group cursor-pointer ${theme === 'dark' ? 'border-[#D7E2EA]/15 bg-[#161616]' : 'border-[#0F172A]/15 bg-slate-100'
                }`}
            >
              <img
                src={imgSrc}
              ...
              <div className="absolute inset-0 bg-gradient-to-t from-black/60 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-end p-4">
                <span className="text-white text-xs font-black uppercase tracking-wider flex items-center gap-1.5">
                  <ExternalLink className="w-3.5 h-3.5" /> View Project Screenshot
                </span>
              </div>
            </motion.div>
          ))}
